...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!member) return message.channel.send(embed.setDescription("Lütfen bir kullanıcı etiketleyip tekrar deneyiniz.\n `.designer @Ducky/ID`")).then(message.react(client.config.no));

        await message.guild.members.cache.get(member.id).roles.add(client.config.designer).then(message.react(client.config.tik));
        message.channel.send(embed.setDescription(`${member} adlı kullanıcı'ya **DESIGNER** rolü verildi!`)
